...
export default function ConsultaPapeletaPage() {
  ...
  const [papeletaActual, setPapeletaActual] = useState({
    ...
    falta: "G40",
    descripcionFalta:
      "Circular a una velocidad superior a la máxima permitida, según la señalización o la que establece el Reglamento.",
    fecha: "27/12/2025",
    importe: "880.00",
    descuento: "730.40",
    deuda: "149.60",
  ...
  const pasosPEC = [
    ...
    },
    {
      id: 6,
      title: t(
        "platform.consultation.timeline.pec.precautionaryMeasures.title",
      ),
        ...
        registerCommand({
          ...
          action: (transcript) => {
            ...
            const query = normalized
              .replace(
                /^(buscar\s*(papeleta|placa)?|consultar|quiero buscar|buscame|consultame|checar|chequear|ver|revisar|averiguar|maskay|papeleta\s*maskay|placa\s*maskay|tapukuy|qaway|qatipay)\s*/,
                "",
              )
              .trim();
            if (query) {
        ...
        registerCommand({
          ...
          action: (transcript) => {
            ...
            const dni = normalized
              .replace(
                /^(buscar\s*dni|consultar\s*dni|buscar\s*por\s*documento|consultar\s*documento|ver\s*por\s*dni|revisar\s*con\s*dni|dni\s*maskay|dni\s*tapukuy|documentowan\s*maskay|maskay\s*dni|tapukuy\s*dni)\s*/,
                "",
              )
              .trim();
            if (dni) {
      ...
      cleanups.push(
        registerCommand({
          ...
          action: () => {
            if (showPhoto) {
              closeAllModals();
              speak(t("voice.closingPhoto"));
              return;
            }
            if (reclamoOpen) {
              closeAllModals();
              speak(t("voice.closingClaim"));
              return;
            }
            if (pagoOpen) {
              closeAllModals();
              speak(t("voice.closingPayment"));
              return;
            }
          },
          scope: "consulta-papeleta",
      ...
      cleanups.push(
        registerCommand({
          ...
          action: () => {
            if (showPhoto) {
              closeAllModals();
              speak(t("voice.photoClosed"));
              return;
            }
            if (reclamoOpen) {
              closeAllModals();
              speak(t("voice.claimClosed"));
              return;
            }
            if (pagoOpen) {
              closeAllModals();
              speak(t("voice.paymentClosed"));
              return;
            }
        ...
        <main className="relative flex min-h-svh flex-col overflow-hidden bg-platform-blue p-6 text-white">
          ...
          <div className="z-20 mx-auto mb-8 flex w-full max-w-6xl flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <Breadcrumb>
              <BreadcrumbList>
                ...
                <BreadcrumbSeparator className="text-white/30" />
                <BreadcrumbItem>
                  <BreadcrumbPage className="text-white font-black uppercase">
                    {t("platform.consultation.ticketsBreadcrumb")}
                  </BreadcrumbPage>
                </BreadcrumbItem>
              </BreadcrumbList>
            </Breadcrumb>
          ...
          <div className="z-10 m-auto w-full max-w-2xl space-y-10 text-center pb-20">
            ...
            <form onSubmit={handleSearch} className="relative group">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1 relative">
                    ...
                    <input
                      ...
                      placeholder={t(
                        "platform.consultation.ticketInputPlaceholder",
                      )}
                  ...
                  >
                    {isLoading ? (
                      ...
                      t("platform.consultation.searchButton")
                    )}
                  </Button>
                </div>
              </div>
              <div className="mt-6 flex flex-wrap justify-center gap-6">
                <Link
                  to="/inicio"
                  className="text-sm font-bold text-white/60 hover:text-white flex items-center gap-2 transition-colors"
                >
                  <ArrowLeft className="size-4" />{" "}
                  {t("platform.consultation.backToMainMenu")}
                </Link>
          ...
          <div className="absolute bottom-8 text-center opacity-40">
            <p className="text-[10px] font-black uppercase tracking-[0.3em]">
              {t("platform.consultation.satFooter")}
            </p>
          </div>
        </main>
      )}
        ...
        <div className="min-h-screen bg-[radial-gradient(circle_at_10%_0%,#e2f1ff_0,transparent_34%),linear-gradient(180deg,#fbfdff,#f3f7fb)] p-4 md:p-8">
          <div className="mx-auto max-w-6xl pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <Breadcrumb>
                <BreadcrumbList>
                  <BreadcrumbItem>
                    <BreadcrumbLink asChild>
                      <Link to="/inicio">{t("common.home")}</Link>
                    </BreadcrumbLink>
                  </BreadcrumbItem>
                  <BreadcrumbSeparator />
                  <BreadcrumbItem>
                    <BreadcrumbLink asChild>
                      <Link to="/consulta-en-linea">
                        {t("platform.consultation.title")}
                      </Link>
                    </BreadcrumbLink>
                  </BreadcrumbItem>
                  ...
                  <BreadcrumbItem>
                    <BreadcrumbPage>
                      {t("platform.consultation.trackingBreadcrumb", {
                        ticket: papeletaActual.nro,
                      })}
                    </BreadcrumbPage>
                  </BreadcrumbItem>
                </BreadcrumbList>
            ...
            <section className="mb-5 grid gap-5 lg:grid-cols-[1fr_minmax(320px,420px)] lg:items-end">
              <div>
                <h1 className="text-2xl font-black uppercase leading-none tracking-tight text-[#073b72] sm:text-3xl md:text-4xl">
                  {t("platform.consultation.trackingTitle")}
                </h1>
                <p className="mt-2.5 text-sm font-extrabold text-[#2f6da8]">
                  {t("platform.consultation.trackingDescription", {
                    plate: papeletaActual.placa,
                  })}
                </p>
              </div>
              ...
              <form
                ...
                className="flex h-14 items-center gap-2 rounded-[20px] border border-[#dbe7f4] bg-white/95 p-1.5 shadow-[0_16px_32px_rgba(15,45,82,.08)]"
              >
                <input
                  type="text"
                  value={searchQuery}
                  ...
                  placeholder={t(
                    "platform.consultation.ticketSearchPlaceholder",
                  )}
                  className="h-full min-w-0 flex-1 rounded-[14px] border-0 bg-[#f8fbff] px-4 text-sm font-black uppercase text-[#073b72] outline-none placeholder:text-slate-400"
                  disabled={isLoading}
                />
                <Button
                  type="button"
                  onClick={resetSearch}
                  disabled={isLoading}
                  className="h-full rounded-[14px] bg-platform-blue px-3 text-[11px] font-black text-white hover:bg-platform-blue/90 sm:px-6 sm:text-xs"
                >
                  {isLoading ? (
                    <Loader2 className="size-4 animate-spin" />
                  ) : (
                    t("platform.consultation.searchAnother")
                  )}
                </Button>
              </form>
            </section>
            ...
            <section className="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
              <article className="flex min-h-[72px] flex-col justify-center rounded-[18px] border border-slate-200 bg-white/95 p-4 shadow-[0_12px_26px_rgba(15,45,82,.06)]">
                <p className="mb-1.5 text-[10px] font-black uppercase tracking-[0.12em] text-slate-400">
                  {t("platform.consultation.ticketNumber")}
                </p>
                <p className="text-xl font-black leading-none text-[#073b72] sm:text-2xl">
                  {papeletaActual.nro}
                </p>
              </article>
              <article className="group relative flex min-h-[72px] flex-col justify-center overflow-hidden rounded-[18px] border border-slate-200 bg-white/95 p-4 shadow-[0_12px_26px_rgba(15,45,82,.06)]">
                <p className="mb-1.5 text-[10px] font-black uppercase tracking-[0.12em] text-slate-400">
                  {t("platform.consultation.vehiclePlate")}
                </p>
                <p className="text-xl font-black leading-none text-[#073b72] sm:text-2xl">
                  {papeletaActual.placa}
                </p>
              ...
              <article className="flex min-h-[72px] flex-col justify-center rounded-[18px] border border-slate-200 bg-white/95 p-4 shadow-[0_12px_26px_rgba(15,45,82,.06)]">
                <p className="mb-1.5 text-[10px] font-black uppercase tracking-[0.12em] text-slate-400">
                  {t("platform.consultation.originalAmount")}
                </p>
                <p className="text-xl font-black leading-none text-[#073b72] sm:text-2xl">
                  S/ {papeletaActual.importe}
                </p>
              </article>
              <article className="relative flex min-h-[72px] flex-col justify-center overflow-hidden rounded-[18px] border border-green-300 bg-gradient-to-br from-green-50 to-white p-4 shadow-[0_12px_26px_rgba(15,45,82,.06)] after:absolute after:right-3 after:top-3 after:rounded-full after:border after:border-green-200 after:bg-green-100 after:px-2.5 after:py-1 after:text-[11px] after:font-black after:text-green-700 after:content-['83%']">
                <p className="mb-1.5 text-[10px] font-black uppercase tracking-[0.12em] text-green-700">
                  {t("platform.consultation.totalToPay")}
                </p>
                <p className="text-2xl font-black leading-none text-green-600 sm:text-3xl">
                  S/ {papeletaActual.deuda}
                </p>
                <p className="mt-1.5 text-[11px] font-bold text-green-700/80">
                  {t("platform.consultation.savingToday", {
                    amount: papeletaActual.descuento,
                  })}
                </p>
              </article>
            </section>
            ...
            <section className="mb-5 flex flex-col gap-4 rounded-[22px] border border-slate-200 border-l-4 border-l-platform-blue bg-white p-5 shadow-[0_14px_30px_rgba(15,45,82,.06)] sm:flex-row sm:items-center sm:justify-between">
              <div className="min-w-0 space-y-1.5">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-[11px] font-black uppercase tracking-[0.14em] text-slate-500">
                    {t("platform.consultation.infractionReason")}
                  </span>
                  <strong className="rounded-full bg-[#073b72] px-3 py-1 text-xs font-black text-white">
                    {papeletaActual.falta}
                  </strong>
                </div>
              ...
              <Button
                ...
                className="shrink-0 border-platform-blue font-black text-platform-blue hover:bg-blue-50"
              >
                <Eye className="size-4" />{" "}
                {t("platform.consultation.viewEvidence")}
              </Button>
            </section>
            ...
            <section className="overflow-hidden rounded-[30px] border border-[#dde8f3] bg-white shadow-[0_24px_60px_rgba(18,50,86,.12)]">
              <div className="flex flex-col gap-4 bg-gradient-to-r from-platform-blue to-[#2469ad] px-6 py-5 text-white sm:flex-row sm:items-center sm:justify-between">
                <h2 className="flex items-center gap-3 text-lg font-black uppercase tracking-widest">
                  ...
                  {t("platform.consultation.procedureRoute")}
                </h2>
                <div className="flex flex-wrap justify-start gap-2 text-[11px] font-black sm:justify-end">
                  <span className="rounded-full border border-blue-100 bg-white px-3 py-2 text-platform-blue shadow-sm">
                    {t("platform.consultation.activeStage")}:{" "}
                    {t("platform.consultation.timeline.pas.notification.title")}
                  </span>
                  <span className="rounded-full border border-amber-200 bg-amber-50 px-3 py-2 text-amber-700 shadow-sm">
                    {t("platform.consultation.due")}:{" "}
                    {t("platform.consultation.deadlineDateShort")}
                  </span>
                </div>
              </div>
              ...
              <div className="grid gap-6 p-5 md:p-8 lg:p-10">
                <section className="grid gap-5 rounded-[26px] border border-dashed border-[#bad1ea] bg-gradient-to-br from-[#f8fbff] to-white p-6 lg:grid-cols-[64px_1fr_auto] lg:items-center">
                  <div className="mx-auto flex size-16 items-center justify-center rounded-[20px] bg-platform-blue text-white shadow-[0_16px_28px_rgba(23,79,143,.22)] sm:mx-0">
                    <AlertTriangle className="size-8" />
                  </div>
                  <div className="space-y-1.5 text-center sm:text-left">
                    <h3 className="text-2xl font-black text-[#073b72]">
                      {t("platform.consultation.requiredAction")}
                    </h3>
                    <p className="text-sm font-medium leading-relaxed text-slate-600">
                      {t("platform.consultation.requiredActionDescription", {
                        date: t("platform.consultation.requiredActionDate"),
                        discount: "83%",
                      })}
                    </p>
                  </div>
                  <div className="grid gap-3 sm:grid-cols-2 lg:min-w-[204px] lg:grid-cols-1">
                    <Button
                      ...
                      className="h-auto rounded-xl bg-platform-blue px-6 py-4 text-base font-black shadow-[0_14px_26px_rgba(23,79,143,.24)] hover:-translate-y-0.5 hover:bg-platform-blue/90"
                    >
                      {t("platform.consultation.payNow")}{" "}
                    ...
                    <Button
                      ...
                      className="h-auto rounded-xl border-platform-blue bg-white px-6 py-4 font-black text-platform-blue hover:-translate-y-0.5 hover:bg-blue-50"
                    >
                ...
                <section className="relative overflow-visible rounded-[30px] border border-[#dce8f4] bg-[linear-gradient(90deg,rgba(18,183,106,.08)_0_70%,rgba(245,158,11,.11)_70%_100%),linear-gradient(180deg,#ffffff,#f8fbff)] p-6 shadow-[0_20px_48px_rgba(15,45,82,.08)] before:absolute before:inset-x-0 before:top-0 before:h-1.5 before:rounded-t-[30px] before:bg-[linear-gradient(90deg,#12b76a_0_42%,#174f8f_42%_48%,#dbe6f2_48%_70%,#f59e0b_70%_100%)]">
                  <div className="mb-5">
                    <h3 className="text-sm font-black uppercase tracking-[0.13em] text-[#073b72]">
                      {t("platform.consultation.procedureProgress")}
                    </h3>
                  </div>

                  <div className="mb-5 hidden grid-cols-[70%_30%] text-center md:grid">
                    <span className="mx-auto rounded-full bg-green-100 px-6 py-2 text-[11px] font-black uppercase tracking-[0.26em] text-green-700 shadow-sm">
                      {t("platform.consultation.timeline.pasLabel")}
                    </span>
                    <span className="mx-auto rounded-full bg-orange-100 px-6 py-2 text-[11px] font-black uppercase tracking-[0.2em] text-orange-700 shadow-sm">
                      {t("platform.consultation.timeline.pecLabel")}
                    </span>
                  </div>

                  <div
                    className="relative flex overflow-x-auto pb-28 pt-2 md:overflow-visible md:pb-8"
                    aria-label={t("platform.consultation.procedureRoute")}
                  >
                    {[...pasosPAS, ...pasosPEC].map((paso, idx, steps) => (
                      <div
                        key={paso.id}
                        className="flex min-w-[120px] flex-1 items-start"
                      >
                        <div
                          tabIndex={0}
                          className="group relative z-10 flex min-w-[98px] flex-col items-center text-center outline-none"
                        >
                          <div
                            className={cn(
                              ...
                              paso.status === "completed" &&
                                "border-green-500 bg-gradient-to-br from-green-500 to-green-600 text-white shadow-green-200",
                              paso.status === "current" &&
                                "-mt-1 size-[74px] rounded-[26px] border-white bg-gradient-to-br from-[#073b72] to-[#1f6db8] text-white shadow-[0_0_0_8px_rgba(23,79,143,.10),0_18px_30px_rgba(23,79,143,.28)]",
                              paso.status === "upcoming" &&
                                paso.id >= 5 &&
                          ...
                          <p
                            className={cn(
                              "mt-3 max-w-[112px] text-xs font-black leading-tight",
                              paso.status === "current"
                                ? "text-[#073b72]"
                                : "text-slate-500",
                            )}
                          ...
                          <div className="pointer-events-none absolute left-1/2 top-[116px] z-30 w-56 -translate-x-1/2 translate-y-2 rounded-[18px] border border-[#dbe6f2] bg-white p-3.5 text-left opacity-0 shadow-[0_18px_42px_rgba(15,45,82,.16)] transition-all duration-200 before:absolute before:left-1/2 before:-top-2 before:size-4 before:-translate-x-1/2 before:rotate-45 before:border-l before:border-t before:border-[#dbe6f2] before:bg-white before:content-[''] group-hover:translate-y-0 group-hover:opacity-100 group-focus:translate-y-0 group-focus:opacity-100">
                            <strong className="mb-1.5 block text-sm font-black text-[#073b72]">
                              {paso.title}
                            </strong>
                            <p className="m-0 text-xs font-medium leading-relaxed text-slate-600">
                              {paso.desc}
                            </p>
                          </div>
                        </div>
                          ...
                          <div
                            className={cn(
                              ...
                              paso.status === "completed" &&
                                ...
                                "bg-gradient-to-r from-green-500 via-green-300 to-platform-blue after:absolute after:inset-0 after:-translate-x-full after:bg-[linear-gradient(90deg,transparent,rgba(255,255,255,.45),transparent)] after:content-['']",
                              idx >= 4 &&
                ...
                <section className="grid gap-6 lg:grid-cols-[minmax(320px,.85fr)_minmax(0,1.15fr)]">
                  <article className="rounded-[26px] bg-gradient-to-br from-[#073b72] to-platform-blue p-7 text-white shadow-[0_24px_45px_rgba(23,79,143,.22)]">
                    <p className="mb-3 text-xs font-black uppercase tracking-[0.14em] text-white/70">
                      {t("platform.consultation.youAreHere")}
                    </p>
                    <h3 className="mb-3 text-3xl font-black">
                      {t(
                        "platform.consultation.timeline.pas.notification.title",
                      )}
                    </h3>
                    <p className="mb-5 text-sm font-medium leading-relaxed text-blue-100">
                      {t("platform.consultation.currentStageDescription")}
                    </p>
                    <div className="mb-5 flex justify-between gap-4 rounded-[18px] border border-white/20 bg-white/10 p-4">
                      <div>
                        <small className="text-white/70">
                          {t("platform.consultation.deadline")}
                        </small>
                      ...
                      <div>
                        <small className="text-white/70">
                          {t("platform.consultation.discount")}
                        </small>
                  ...
                  <article className="rounded-[26px] border border-[#dce8f4] bg-white p-5 shadow-[0_18px_38px_rgba(15,45,82,.08)]">
                    <div className="mb-4 flex items-start justify-between gap-3">
                      <div>
                        <h3 className="text-base font-black text-[#073b72]">
                          {t("platform.consultation.caseDocuments")}
                        </h3>
                        <p className="mt-1 text-xs font-semibold text-slate-500">
                          {t("platform.consultation.caseDocumentsDescription")}
                        </p>
                      </div>
                      <span className="rounded-full bg-blue-50 px-3 py-2 text-[11px] font-black text-platform-blue">
                        {t("platform.consultation.swipe")}
                      </span>
                    </div>
                    ...
                    <div className="grid auto-cols-[minmax(240px,42%)] grid-flow-col gap-4 overflow-x-auto pb-3">
                      <button
                        onClick={() => setShowPhoto(true)}
                        className="group relative min-h-[330px] overflow-hidden rounded-[22px] border border-[#dce8f4] bg-slate-50 text-left shadow-[0_14px_28px_rgba(15,45,82,.08)]"
                      >
                        <img
                          src={infractionImage}
                        ...
                        <span className="absolute inset-x-4 bottom-4 grid gap-1 rounded-2xl bg-[#073b72]/95 p-4 text-white">
                          <strong className="text-sm font-black">
                            {t("platform.consultation.originalTicket")}
                          </strong>
                          <small className="text-xs opacity-80">
                            {t("platform.consultation.clickToExpand")}
                          </small>
                        </span>
                      </button>
                      ...
                      <article className="relative flex min-h-[330px] flex-col justify-end gap-2 rounded-[22px] border border-[#dce8f4] bg-[radial-gradient(circle_at_30%_18%,rgba(23,79,143,.18),transparent_34%),linear-gradient(145deg,#ffffff,#eef6ff)] p-6 shadow-[0_14px_28px_rgba(15,45,82,.08)]">
                        <div className="absolute left-6 top-6 grid size-14 place-items-center rounded-[20px] bg-white text-platform-blue shadow-[0_12px_24px_rgba(15,45,82,.10)]">
                          <FileText className="size-7" />
                        </div>
                        <strong className="text-sm font-black text-[#073b72]">
                          {t("platform.consultation.adminReport")}
                        </strong>
                        <small className="text-xs font-semibold text-slate-500">
                          {t("platform.consultation.availableWhenIssued")}
                        </small>
                      </article>
                      ...
                      <article className="relative flex min-h-[330px] flex-col justify-end gap-2 rounded-[22px] border border-[#dce8f4] bg-[radial-gradient(circle_at_30%_18%,rgba(255,179,92,.25),transparent_34%),linear-gradient(145deg,#ffffff,#fff5e8)] p-6 shadow-[0_14px_28px_rgba(15,45,82,.08)]">
                        <div className="absolute left-6 top-6 grid size-14 place-items-center rounded-[20px] bg-white text-orange-500 shadow-[0_12px_24px_rgba(15,45,82,.10)]">
                          <Camera className="size-7" />
                        </div>
                        <strong className="text-sm font-black text-[#073b72]">
                          {t("platform.consultation.photoEvidence")}
                        </strong>
                        <small className="text-xs font-semibold text-slate-500">
                          {t("platform.consultation.addedIfApplies")}
                        </small>
                      </article>
                    </div>
                  </article>
              ...
              <div className="w-full md:w-80 p-8 space-y-6 bg-white border-l">
                <div className="space-y-3">
                  <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">
                    {t("platform.consultation.infractionDescription")}
                  </p>
                  <div className="space-y-2">
                    <h4 className="text-lg font-black text-platform-blue uppercase leading-tight">
                      {papeletaActual.falta}
                    </h4>
                ...
                <div className="space-y-4">
                  <div className="flex justify-between text-sm border-b pb-2">
                    <span className="text-zinc-500 font-medium">
                      {t("platform.consultation.date")}
                    </span>
                    <span className="font-bold text-platform-blue">
                      {papeletaActual.fecha}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm border-b pb-2">
                    <span className="text-zinc-500 font-medium">
                      {t("platform.consultation.location")}
                    </span>
                    <span className="font-bold text-platform-blue text-right">
                      Av. Javier Prado
                    </span>
                  </div>
                </div>
                <div className="pt-4">
                  <div className="p-4 rounded-xl bg-green-50 border border-green-100 text-center">
                    <p className="text-[10px] font-black text-green-700 uppercase mb-1">
                      {t("platform.consultation.status")}
                    </p>
                    <p className="text-sm font-black text-green-600 uppercase italic">
                      {t("platform.consultation.verifiedRecord")}
                    </p>
                  </div>
                </div>
              </div>
